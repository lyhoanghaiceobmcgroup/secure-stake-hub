...
import { CheckCircle, Download, Bell, Eye, QrCode, Copy, Calendar, FileText, ShieldCheck, ExternalLink, ArrowRight } from 'lucide-react';
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <Card className="border-green-200 bg-green-50/50">
            <CardHeader className="pb-3 text-stone-700">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-white rounded-lg border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => copyToClipboard(investmentData.cqidNumber, 'CQĐĐT-ID')}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <Button onClick={onViewPortfolio} className="h-auto p-4 flex flex-col items-center gap-2 bg-blue-600 hover:bg-blue-700">
                <Eye className="w-5 h-5" />
                <span className="text-sm font-medium">Xem Sổ mới</span>
                <span className="text-xs opacity-90">Chi tiết sổ đầu tư</span>
              </Button>
              
              <Button variant="outline" onClick={downloadContract} className="h-auto p-4 flex flex-col items-center gap-2">
                <Download className="w-5 h-5" />
                <span className="text-sm font-medium">Tải hợp đồng</span>
                <span className="text-xs text-muted-foreground">PDF + hash</span>
              </Button>
              
              <Button variant="outline" onClick={setupDistributionReminder} className="h-auto p-4 flex flex-col items-center gap-2">
